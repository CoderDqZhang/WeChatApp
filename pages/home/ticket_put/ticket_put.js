...
Page({
  ...
  changeData: function (ticket) {
    var that = this 
    this.genderData(ticket.session_list[0].ticket_list)
    // that.setData({
    //   ticket_list: that.data.temp_ticket_list.concat(ticket),
    //   temp_ticket_list: that.data.ticket_list.concat(ticket)
    // })
  },
  ...
  onLoad: function (opt) {
    ...
    if (opt.sellShow != null) {
      ...
      this.genderData(this.data.sessionShow.session_list[0].ticket_list)
    }
  },
  ...
  requestDeleteTicket: function (ticket) {
    ...
    app.func.requestDelete(url, {}, function (res) {
      ...
      var tempTicket = []
      for (var j = 0; j < that.data.temp_ticket_list.length; j++) {

        if (that.data.temp_ticket_list[j].id != ticket.id) {
          tempTicket.push(that.data.temp_ticket_list[j])
        }
      }
      that.setData({
        'that.data.showDesc.ticket_list': tempTicket,
        ticket_list: tempTicket,
        temp_ticket_list: tempTicket
      })
    });
  },
  ...
  nextTap: function (){
    ...
    for (var i = 0; i < pages.length; i++) {
      if (pages[i].route == 'pages/home/sell_form/sell_form') {
        var prePage = pages[i];
        //关键在这里
        wx.navigateBack({
    ...
    that.data.sessionShow.cover_end = arr[1]
    wx.navigateTo({
      url: '../sell_form/sell_form?sellShow=' + JSON.stringify(that.data.sessionShow),
    })
  },
